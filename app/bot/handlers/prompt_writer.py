...
async def handle_prompt_writer_text(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        # ОЧИЩАЕМ состояние "Написать" - выходим из PromptWriterStates.waiting_input
        # Это позволяет пользователю нажать "Создать" без необходимости сбрасывать сессию
        await state.set_state(None)
        
        await message.answer(
            f"✅ Промпт сгенерирован:\n```\n{generated_prompt}\n```\n\n"
            f"💡 Теперь нажмите кнопку **«🎨 Создать»** для генерации изображения.",
            parse_mode="Markdown",
            reply_markup=build_main_keyboard(),
        )
