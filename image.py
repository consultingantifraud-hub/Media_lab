...
async def _enqueue_image_edit_task(
    ...
    if operation_id is None:
        ...
        try:
            ...
            user, _ = BillingService.get_or_create_user(db, user_id, message.from_user)
            price = get_operation_price("edit")
    ...
    try:
        ...
        if prompt:
            await message.answer(
                ...
                "• **Seedream** — топовая нейросеть, пишет текст только на английском языке",
...
async def handle_format_choice(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if not format_id:
            logger.debug("handle_format_choice: not a format button, ignoring")
            return
        ...
        if smart_merge_stage == "await_size":
            ...
            # Для Nano Banana Pro edit показываем выбор качества, для остальных - сразу переходим к сбору
            if model_path == SMART_MERGE_PRO_MODEL:
                # Сохраняем формат и переходим к выбору качества
                await state.update_data(
                    {
                        SMART_MERGE_STAGE_KEY: "await_quality",
                ...
                await message.answer(
                    f"Выбран формат: {format_spec.label}\n\n"
                    "Выберите режим качества:\n"
                    "⚡ **Быстрее** — время генерации 1-2 минуты\n"
                    "🎨 **Качественнее** — время генерации 3-4 минуты",
                    reply_markup=build_quality_keyboard(),
                    parse_mode="Markdown",
                )
            else:
                # Для других моделей сразу переходим к сбору изображений
                await state.update_data(
                    {
...
async def _enqueue_smart_merge_task(
    ...
    *,
    ...
    quality_num_inference_steps = data.get("quality_num_inference_steps")  # Параметры качества для Nano Banana Pro edit
    quality_guidance_scale = data.get("quality_guidance_scale")
    ...
    # Если есть параметры качества (для Nano Banana Pro edit), добавляем их в options_override
    if quality_num_inference_steps is not None:
        if not options_override:
            options_override = {}
        options_override["num_inference_steps"] = quality_num_inference_steps
        logger.info("_enqueue_smart_merge_task: using quality num_inference_steps from state: {}", quality_num_inference_steps)
    if quality_guidance_scale is not None:
        if not options_override:
            options_override = {}
        options_override["guidance_scale"] = quality_guidance_scale
        logger.info("_enqueue_smart_merge_task: using quality guidance_scale from state: {}", quality_guidance_scale)
...
async def handle_prompt_input(message: types.Message, state: FSMContext) -> None:
    ...
    # Проверяем, не находимся ли мы в состоянии оплаты или ввода промокода для операций
    ...
    # КРИТИЧЕСКИ ВАЖНО: Если есть smart_merge_stage == "collect", но нет источников - это старое состояние
    # Smart Merge в стадии "collect" ВСЕГДА требует наличия источников
    # Если источников нет - это режим СОЗДАНИЯ, а не Smart Merge - очищаем состояние СРАЗУ
    # Проверяем ДО всех остальных проверок Smart Merge
    if smart_merge_stage == "collect" and len(sources) == 0:
        logger.warning("handle_prompt_input: clearing stale Smart Merge state (collect without sources) - this is CREATE mode. selected_model='{}', smart_merge_stage='{}', smart_merge_model='{}'", 
    ...
    # (есть модель Smart Merge И нет selected_model для создания)
    if smart_merge_stage == "await_model" and smart_merge_model and not selected_model:
        # Пользователь должен выбрать модель для объединения
        logger.info("handle_prompt_input: detected await_model stage, showing model selection")
        await message.answer(
            "Выберите модель:\n"
            ...
            "• **Seedream edit** — топовая нейросеть, пишет текст только на английском языке",
    ...
    if smart_merge_stage == "await_size" and smart_merge_model and not selected_model:
        # Пользователь должен выбрать формат изображения для Nano Banana Pro
        logger.info("handle_prompt_input: detected await_size stage, showing format selection")
        await message.answer(
...
async def handle_smart_merge_start(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer(
            ...
            "• Объединить изображения — отправьте до 4 изображений и опишите сцену\n"
            "• Добавить текст — «добавь текст 'Привет' в центре»\n\n"
            "**Выберите модель:**\n"
            "• **Nano Banana Pro edit** — лучшая нейросеть, в т.ч. работает с длинными текстами на кириллице\n"
            "• **Nano Banana edit** — топовая нейросеть, пишет только заголовки на кириллице\n"
            "• **Seedream edit** — топовая нейросеть, пишет текст только на английском языке\n\n"
            "После выбора модели отправьте до 4 изображений (или одно для редактирования).",
            reply_markup=build_smart_merge_model_keyboard(),
            parse_mode="Markdown",
        )
    except Exception as exc:
...
async def handle_smart_merge_model_choice(message: types.Message, state: FSMContext) -> None:
    ...
    if selection not in {IMAGE_SMART_MERGE_PRO_BUTTON, IMAGE_SMART_MERGE_NANO_BUTTON, IMAGE_SMART_MERGE_SEEDREAM_BUTTON}:
        logger.warning("handle_smart_merge_model_choice: selection '{}' not in smart merge buttons (expected: {}, {} or {}), ignoring", 
                      selection, IMAGE_SMART_MERGE_PRO_BUTTON, IMAGE_SMART_MERGE_NANO_BUTTON, IMAGE_SMART_MERGE_SEEDREAM_BUTTON)
        return
    ...
    try:
        # Для всех моделей (Nano Banana Pro, Nano Banana, Seedream) сначала устанавливаем стадию "await_size" для выбора формата
        await state.update_data(
            {
        ...
        # Проверяем, что стадия сохранилась
        ...
        return  # Важно: не продолжаем выполнение для всех моделей
...
def register_image_handlers(dp: Dispatcher) -> None:
    ...
    # Проверка состояний оплаты выполняется внутри handle_prompt_input
    dp.message.register(
        handle_prompt_input, 
        lambda msg: msg.text and not msg.text.startswith("/") and msg.text != BALANCE_BUTTON
    )
