...
async def handle_create(message: types.Message, state: FSMContext) -> None:
    ...
    if prompt:
        ...
        await message.answer(
            ...
            "• Nano-banana — топовая модель, рисует текст на русском\n"
            "• Nano Banana Pro — лучшее качество кириллицы, продвинутая генерация изображений с текстом\n"
            "• Seedream (Create) — топовая модель, рисует текст только на английском\n"
            "• Flux Ultra — генерирует изображения в максимальном разрешении, рисует текст только на английском",
...
async def handle_gpt_create(message: types.Message, state: FSMContext) -> None:
    ...
    # Сразу запускаем генерацию без выбора размера (фиксированный размер 1024x1024)
    await message.answer("🔄 Генерирую изображение через Nano Banana Pro (лучшее качество кириллицы)...")
    
    # Используем enqueue_image для постановки задачи в очередь
...
async def handle_prompt_input(message: types.Message, state: FSMContext) -> None:
    ...
    # ВАЖНО: включаем IMAGE_LIGHT_BUTTON и IMAGE_STANDARD_BUTTON, чтобы они обрабатывались своими handlers
    main_menu_buttons = {
        CREATE_BUTTON,
        IMAGE_EDIT_BUTTON,
        IMAGE_SMART_MERGE_BUTTON,
    ...
    # Игнорируем кнопки выбора модели - они обрабатываются handle_light и handle_standard
    if text == IMAGE_LIGHT_BUTTON or text == IMAGE_STANDARD_BUTTON or text == IMAGE_SEEDREAM_CREATE_BUTTON or text == IMAGE_GPT_CREATE_BUTTON:
        return
...
async def handle_edit_media(message: types.Message, state: FSMContext) -> None:
    ...
    # Проверяем, не находимся ли мы в режиме Designer Text
    from app.bot.handlers.designer_text import DesignerTextStates
    if current_state == DesignerTextStates.waiting_image.state:
        logger.debug("Skipping handle_edit_media - in Designer Text mode (waiting_image)")
        # Пропускаем обработку - пусть обрабатывает designer_text handler
        return
...
def register_image_handlers(dp: Dispatcher) -> None:
    ...
    # Это гарантирует, что они имеют приоритет при обработке кнопок "Nano-banana" и "Flux Ultra" после "Создать"
    dp.message.register(handle_create, _match_button(CREATE_BUTTON))
    dp.message.register(handle_light, _match_button(IMAGE_LIGHT_BUTTON))
