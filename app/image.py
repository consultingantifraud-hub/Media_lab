...
async def handle_create(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if prompt:
            # Удаляем старую клавиатуру перед отправкой новой
            await message.answer("Выберите модель:", reply_markup=ReplyKeyboardRemove())
...
async def handle_prompt_input(message: types.Message, state: FSMContext) -> None:
    ...
    if smart_merge_stage == "collect" and smart_merge_model is not None and sources and len(sources) > 0 and not selected_model:
        ...
        # (убрали выбор качества, так как разница во времени генерации незначительна)
            ...
            return
    if smart_merge_stage:
        logger.warning("handle_prompt_input: smart_merge_stage='{}' is set but should be None for CREATE mode! Clearing it.", smart_merge_stage)
        # Очищаем состояние Smart Merge, так как пользователь нажимает "Создать", а не "Изменить"
        await state.update_data(
            SMART_MERGE_STAGE_KEY=None,
            SMART_MERGE_SOURCES_KEY=None,
            SMART_MERGE_SIZE_KEY=None,
        ...
        logger.info("handle_prompt_input: Smart Merge state cleared, continuing with CREATE mode")
        # НЕ возвращаемся - продолжаем обработку для режима создания
    ...
    if prompt and not selected_model:
        ...
        logger.info("handle_prompt_input: EARLY RETURN - sending model selection from line 2380-2392")
        model_hint_text = (
        ...
        logger.info("handle_prompt_input: EARLY RETURN MODEL_TEXT contains 'Flux 2 Flex': %s", 'Flux 2 Flex' in model_hint_text)
        logger.info("handle_prompt_input: EARLY RETURN MODEL_TEXT contains 'Seedream 4.5': %s", 'Seedream 4.5' in model_hint_text)
        logger.info("handle_prompt_input: EARLY RETURN MODEL_TEXT: %s", model_hint_text[:200])
        await message.answer(
            model_hint_text,
            reply_markup=build_create_model_keyboard(),
            parse_mode="Markdown",
    ...
    logger.info("handle_prompt_input: ABOUT TO SEND MODEL SELECTION MESSAGE - this log should appear!")
    logger.info("handle_prompt_input: Current file path: %s", __file__)
    
    # Сразу показываем выбор моделей после принятия промпта
    # Порядок моделей (сверху вниз): 1. Nano Banana Pro, 2. Flux 2 Flex, 3. Nano Banana, 4. Seedream 4.5
    model_text = (
        "Выберите модель для создания изображения:\n"
        "• **Nano Banana Pro** — лучшая нейросеть, в т.ч. работает с длинными текстами на кириллице\n"
        "• **Flux 2 Flex** — быстрая и качественная нейросеть, поддерживает кириллицу\n"
    ...
    # ДЕТАЛЬНОЕ ЛОГИРОВАНИЕ ТЕКСТА ПЕРЕД ОТПРАВКОЙ
    logger.info("handle_prompt_input: MODEL_TEXT CONTENT (first 500 chars): %s", model_text[:500])
    logger.info("handle_prompt_input: MODEL_TEXT contains 'Flux 2 Flex': %s", 'Flux 2 Flex' in model_text)
    logger.info("handle_prompt_input: MODEL_TEXT contains 'Seedream 4.5': %s", 'Seedream 4.5' in model_text)
    logger.info("handle_prompt_input: MODEL_TEXT contains 'Seedream' (without 4.5): %s", 'Seedream' in model_text and 'Seedream 4.5' not in model_text)
    
    keyboard = build_create_model_keyboard()
    logger.info("handle_prompt_input: sending model selection message with Seedream 4.5 and Flux 2 Flex, keyboard buttons: %s", 
                [[btn.text for btn in row] for row in keyboard.keyboard])
    
    # Удаляем старую клавиатуру перед отправкой новой, чтобы Telegram обновил кэш
    logger.info("handle_prompt_input: Sending 'Промпт принят ✅.' message")
    await message.answer("Промпт принят ✅.", reply_markup=ReplyKeyboardRemove())
    await asyncio.sleep(0.1)  # Небольшая задержка для обработки удаления
    
    logger.info("handle_prompt_input: ABOUT TO SEND MODEL SELECTION MESSAGE - model_text length: %d", len(model_text))
    logger.info("handle_prompt_input: FINAL MODEL_TEXT TO SEND: %s", model_text)
    await message.answer(
        model_text,
        reply_markup=keyboard,
        parse_mode="Markdown",
    )
    logger.info("handle_prompt_input: MODEL SELECTION MESSAGE SENT SUCCESSFULLY")
