# Настройка SSH подключения к VPS серверу

## 📋 Быстрая настройка

### 1. Генерация SSH ключа (если еще нет)

#### Windows (PowerShell):
```powershell
# Проверить наличие ключа
ls ~/.ssh/id_rsa.pub

# Если ключа нет, создать новый
ssh-keygen -t rsa -b 4096 -C "your_email@example.com"

# Скопировать публичный ключ в буфер обмена
Get-Content ~/.ssh/id_rsa.pub | Set-Clipboard
```

#### Linux/WSL:
```bash
# Проверить наличие ключа
ls ~/.ssh/id_rsa.pub

# Если ключа нет, создать новый
ssh-keygen -t rsa -b 4096 -C "your_email@example.com"

# Показать публичный ключ для копирования
cat ~/.ssh/id_rsa.pub
```

### 2. Добавление SSH ключа на сервер

```bash
# Скопировать публичный ключ на сервер
ssh-copy-id username@your-server-ip

# Или вручную:
cat ~/.ssh/id_rsa.pub | ssh username@your-server-ip "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"
```

### 3. Настройка SSH конфигурации

#### Windows:
1. Создайте директорию (если нет): `mkdir C:\Users\<username>\.ssh`
2. Скопируйте `ssh_config.example` в `C:\Users\<username>\.ssh\config`
3. Отредактируйте файл, заменив:
   - `YOUR_SERVER_IP_OR_DOMAIN` на IP или домен вашего сервера
   - `YOUR_USERNAME` на ваше имя пользователя
   - При необходимости путь к ключу

#### Linux/WSL:
```bash
# Создать директорию (если нет)
mkdir -p ~/.ssh
chmod 700 ~/.ssh

# Скопировать пример конфигурации
cp ssh_config.example ~/.ssh/config

# Отредактировать конфигурацию
nano ~/.ssh/config

# Установить правильные права доступа
chmod 600 ~/.ssh/config
```

### 4. Редактирование конфигурации

Откройте файл `~/.ssh/config` (или `C:\Users\<username>\.ssh\config` на Windows) и замените:

```
Host media-lab-vps
    HostName YOUR_SERVER_IP_OR_DOMAIN      # Например: 192.168.1.100 или server.example.com
    User YOUR_USERNAME                      # Например: root или deploy
    Port 22                                 # Порт SSH (обычно 22)
    IdentityFile ~/.ssh/id_rsa             # Путь к приватному ключу
```

## 🚀 Использование

### Подключение через конфигурацию

После настройки конфигурации можно подключаться просто по имени:

```bash
ssh media-lab-vps
```

### Использование скриптов подключения

#### Windows (PowerShell):
```powershell
# Использовать скрипт подключения
.\scripts\ssh-connect.ps1

# Или указать конкретный сервер
.\scripts\ssh-connect.ps1 -ServerName media-lab-vps

# Или подключиться напрямую с параметрами
.\scripts\ssh-connect.ps1 -ServerIP "192.168.1.100" -Username "deploy" -Port 22
```

#### Linux/WSL:
```bash
# Сделать скрипт исполняемым
chmod +x scripts/ssh-connect.sh

# Использовать скрипт подключения
./scripts/ssh-connect.sh

# Или указать конкретный сервер
./scripts/ssh-connect.sh media-lab-vps
```

### Прямое подключение без конфигурации

```bash
# Базовое подключение
ssh username@server-ip

# С указанием порта
ssh -p 2222 username@server-ip

# С указанием ключа
ssh -i ~/.ssh/id_rsa username@server-ip
```

## 🔒 Безопасность

### Рекомендации:

1. **Используйте SSH ключи вместо паролей**
   ```bash
   # Отключить вход по паролю на сервере (после настройки ключей)
   sudo nano /etc/ssh/sshd_config
   # Установить: PasswordAuthentication no
   sudo systemctl restart sshd
   ```

2. **Измените стандартный порт SSH** (опционально)
   ```bash
   # На сервере отредактировать /etc/ssh/sshd_config
   Port 2222
   ```

3. **Ограничьте доступ по IP** (если возможно)
   ```bash
   # В /etc/ssh/sshd_config
   AllowUsers username@your-ip
   ```

4. **Используйте fail2ban** для защиты от брутфорса
   ```bash
   sudo apt install fail2ban
   sudo systemctl enable fail2ban
   sudo systemctl start fail2ban
   ```

## 📝 Полезные команды SSH

### Копирование файлов

```bash
# Копировать файл на сервер
scp file.txt username@server:/path/to/destination/

# Копировать файл с сервера
scp username@server:/path/to/file.txt ./

# Копировать директорию
scp -r directory/ username@server:/path/to/destination/
```

### Выполнение команд на удаленном сервере

```bash
# Выполнить одну команду
ssh username@server "ls -la"

# Выполнить несколько команд
ssh username@server "cd /opt/media-lab && docker ps"
```

### Туннелирование портов

```bash
# Локальный туннель (доступ к удаленному порту локально)
ssh -L 8000:localhost:8000 username@server

# Удаленный туннель (доступ к локальному порту удаленно)
ssh -R 8000:localhost:8000 username@server
```

## 🐛 Решение проблем

### Ошибка "Permission denied (publickey)"

1. Проверьте права доступа на ключи:
   ```bash
   chmod 600 ~/.ssh/id_rsa
   chmod 644 ~/.ssh/id_rsa.pub
   ```

2. Проверьте, что публичный ключ добавлен на сервер:
   ```bash
   ssh username@server "cat ~/.ssh/authorized_keys"
   ```

3. Проверьте конфигурацию SSH на сервере:
   ```bash
   sudo nano /etc/ssh/sshd_config
   # Убедитесь, что:
   # PubkeyAuthentication yes
   # AuthorizedKeysFile .ssh/authorized_keys
   ```

### Ошибка "Host key verification failed"

```bash
# Удалить старый ключ хоста
ssh-keygen -R server-ip

# Или отредактировать ~/.ssh/known_hosts вручную
```

### Подключение слишком медленное

Добавьте в `~/.ssh/config`:
```
Host *
    GSSAPIAuthentication no
    UseDNS no
```

## 📚 Дополнительные ресурсы

- [OpenSSH документация](https://www.openssh.com/manual.html)
- [SSH ключи - GitHub Guide](https://docs.github.com/en/authentication/connecting-to-github-with-ssh)







