...
def _build_input_payload(prompt: str, options: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Для Seedream используем width и height напрямую
    if "seedream" in model.lower():
        # Seedream принимает width и height как отдельные параметры
        if "width" in options and "height" in options:
            payload["width"] = options.pop("width")
            payload["height"] = options.pop("height")
            logger.info("_build_input_payload: Seedream detected, setting width={}, height={}", 
                       payload["width"], payload["height"])
        # Удаляем size и aspect_ratio, если они есть, так как Seedream использует width/height
...
def submit_image(prompt: str, **opts: Any) -> str:
    ...
    if "seedream" in model.lower():
        logger.info("submit_image: Seedream payload - size='{}', width={}, height={}", 
                   input_payload.get("size"), input_payload.get("width"), input_payload.get("height"))
