...
try:
    from app.providers.fal.images import run_face_swap_replicate
    from app.providers.replicate.client import check_replicate_available
    REPLICATE_AVAILABLE = True
except ImportError:
    REPLICATE_AVAILABLE = False

try:
    from app.providers.wavespeed.client import wavespeed_face_swap
    WAVESPEED_AVAILABLE = True
except ImportError:
    WAVESPEED_AVAILABLE = False
    REPLICATE_AVAILABLE = False
    logger.debug("Replicate provider not available for fallback")
...
RETOUCHER_MODELS = {
    ...
    "enhance": settings.fal_face_enhance_model,
}
...
def _send_success_notification_sync(
    ...
    # Показываем промпт только если он не пустой
    prompt = notify.get("prompt")
    full_caption = None
    if prompt and prompt.strip():
        full_caption = f"{caption_title}\nПромпт: {prompt}"
        # Telegram ограничение: максимум 1024 символа для caption
        # Если промпт слишком длинный, отправляем только заголовок в caption,
        # а полный промпт отправим отдельным сообщением после отправки изображения
        if len(full_caption) > 1024:
            caption = caption_title  # Только заголовок в caption
        else:
            caption = full_caption  # Полный промпт в caption
    else:
    ...
    if image_bytes is not None:
        ...
        # Send URL directly as string - Telegram Bot API will download it server-side
        sent_message_id = send_document_sync(
            chat_id=notify["chat_id"],
            document=image_url,  # Pass URL as string - Telegram downloads it
            caption=caption,
            reply_to_message_id=notify.get("reply_to_message_id"),
            message_thread_id=notify.get("message_thread_id"),
            reply_markup=reply_markup_dict,
    ...
    # Если промпт был обрезан из-за ограничения Telegram (1024 символа), отправляем полный промпт отдельным сообщением
    if full_caption and len(full_caption) > 1024:
        from app.core.telegram_sync import send_message_sync
        send_message_sync(
            chat_id=notify["chat_id"],
            text=full_caption,
            reply_to_message_id=sent_message_id,  # Ответ на отправленное изображение
            message_thread_id=notify.get("message_thread_id"),
        )
...
def process_face_swap_job(
    ...
    is_advanced_model = (
        "easel-ai" in model_name.lower() or 
        "easel/" in model_name.lower() or
    ...
    if is_advanced_model:
        ...
        if not current_settings.wavespeed_api_key:
            error_msg = "WaveSpeedAI API ключ не настроен. Установите WAVESPEED_API_KEY в настройках."
            logger.error("Face swap job {}: WaveSpeedAI API key not configured", job_id)
            if job:
                job.meta["error"] = error_msg
                job.save_meta()
            if notify_options.get("chat_id"):
    ...
    if asset is None:
        ...
        for result_attempt in range(max_result_attempts):
            try:
                ...
                if status_code in (500, 502, 503, 401) and result_attempt < max_result_attempts - 1:
                    ...
                    # Check if it's a 500 error from easel-ai
                    if status_code == 500 and "easel-ai" in model_name.lower():
                        error_msg = (
                            "Ошибка обработки на стороне API Fal.ai для модели Advanced Face Swap.\n\n"
                            "Возможные причины:\n"
        ...
        if asset is None:
            ...
            error_str = str(error)
            if "500" in error_str and "easel-ai" in model_name.lower():
                user_error_msg = (
                    "Ошибка обработки на стороне API Fal.ai для модели Advanced Face Swap.\n\n"
                    "Возможные причины:\n"
                    "• Временная недоступность модели\n"
...
def process_image_job(job_id: str, prompt: str, options: dict | None, output_path: str) -> str:
    ...
    is_nano_banana = model_name == "fal-ai/nano-banana" or "nano-banana" in model_name.lower()
    ...
    if asset is None:
        poll_attempts = 0
        max_attempts = 120  # roughly two minutes with 1s interval
        poll_interval = 1.0
        status: dict[str, Any]
        while True:
            status = check_image_status(task_id)
    ...
    if notify_options.get("chat_id"):
        try:
            ...
            logger.error("Failed to send Telegram notification for job {}: {}", job_id, notify_error)
    return image_url or ""
...
def process_retoucher_job(
    ...
    provider_options: Dict[str, Any] = dict(options or {})
    provider_prompt = provider_options.pop("provider_prompt", prompt)
    model_name = RETOUCHER_MODELS.get(mode, settings.fal_retoucher_model)
    ...
    submit_kwargs = dict(provider_options)
    submit_kwargs.pop("model", None)
    submit_kwargs.pop("preset", None)
    ...
    if image_bytes:
        ...
        try:
            ...
            logger.warning("Retoucher job {}: failed to save inline bytes: {}", job_id, exc)
    elif asset.url:
        # We only have URL - schedule background download for caching
        # but don't try to download synchronously as it may timeout
        # Telegram can download the image directly from the URL
        _schedule_result_download(job_id, asset.url, output_file)
        logger.info("Retoucher job {}: will send image by URL (background download scheduled): {}", job_id, asset.url[:100] if asset.url else "None")
        # Continue without local file - we'll send by URL
    ...
    caption_path = saved_path.as_posix() if saved_path else asset.url or ""
    logger.success("Retoucher job {} completed: {}", job_id, caption_path)

    if notify_options.get("chat_id"):
        logger.info("Retoucher job {}: preparing notification (image_bytes={}, saved_path={}, asset.url={})", 
                    job_id, image_bytes is not None, saved_path, asset.url[:100] if asset.url else None)
        try:
            if image_bytes is None and saved_path and saved_path.exists():
                image_bytes = saved_path.read_bytes()
                filename = filename or saved_path.name
                logger.info("Retoucher job {}: loaded image bytes from saved file ({} bytes)", job_id, len(image_bytes))
        except Exception as exc:  # noqa: BLE001
            logger.warning("Retoucher job {}: failed to prepare bytes for notification: {}", job_id, exc)

        if image_bytes is not None:
            logger.info("Retoucher job {}: sending notification with image bytes ({} bytes)", job_id, len(image_bytes))
            try:
                _send_success_notification_sync(
                    notify_options,
                    job_id,
                    image_bytes=image_bytes,
                    filename=filename or "retouch.png",
                    caption_title="✨ Ретушь готова!",
                    reply_markup=None,
                )
                logger.info("Retoucher job {}: successfully sent notification with image bytes", job_id)
            except Exception as notify_error:  # noqa: BLE001
                logger.error("Failed to send Telegram notification for retoucher job {}: {}", job_id, notify_error)
        elif asset.url:
            # Fallback to sending the URL if bytes are unavailable
            logger.info("Retoucher job {}: sending notification with image URL: {}", job_id, asset.url[:100] if asset.url else "None")
            try:
                _send_success_notification_sync(
                    notify_options,
                    job_id,
                ...
                logger.info("Retoucher job {}: successfully sent notification with image URL", job_id)
            except Exception as notify_error:  # noqa: BLE001
                logger.error("Failed to send fallback notification for retoucher job {}: {}", job_id, notify_error)
        else:
            logger.error("Retoucher job {}: cannot send notification - no image_bytes and no asset.url", job_id)
...
def process_smart_merge_job(
    ...
    # Проверяем, является ли модель Nano-banana (может принимать русский текст)
    model_name = provider_options.get("model", "")
    is_nano_banana = model_name == SMART_MERGE_DEFAULT_MODEL or model_name == "fal-ai/nano-banana" or "nano-banana" in model_name.lower()
        ...
        try:
            ...
            if image_bytes is not None:
                _send_success_notification_sync(
                    ...
                    image_bytes=image_bytes,
                    filename=filename,
                    caption_title="🧠 Smart merge готово!",
                    reply_markup=reply_markup,
                )
            else:
                _send_success_notification_sync(
